...
exports.DEFAULT_POLICIES = exports.PolicyEngine = void 0;
...
class PolicyEngine {
    ...
    evaluate(result) {
        ...
        if (matchedRules.length === 0) {
            ...
            return this.makeDecision('WARN', [], result.detections, `${result.detectionCount} sensitive item(s) found — no policy matched. Proceeding with warning.`);
        }
     ...
     */
    matchesCondition(policy, result) {
        const { conditions } = policy;
        const { detections } = result;
        // Category filter
        if (conditions.categories && conditions.categories.length > 0) {
            const hasMatch = detections.some(d => conditions.categories.includes(d.category));
            if (!hasMatch)
                return false;
        }
        // Severity filter
        if (conditions.severities && conditions.severities.length > 0) {
            const hasMatch = detections.some(d => conditions.severities.includes(d.severity));
            if (!hasMatch)
                return false;
        }
        // Specific rule ID filter
        if (conditions.ruleIds && conditions.ruleIds.length > 0) {
            const hasMatch = detections.some(d => conditions.ruleIds.includes(d.ruleId));
            if (!hasMatch)
                return false;
        }
        // Minimum detection count
        if (conditions.minCount !== undefined) {
            if (result.detectionCount < conditions.minCount)
                return false;
        }
        return true;
    }
    makeDecision(action, matchedRules, detections, message) {
        return {
            action,
            matchedRules,
            detections,
    ...
    buildMessage(action, matchedRules, result) {
     ...
     */
    loadPolicies() {
        const policyFilePath = this.config.get('policyFile');
        if (policyFilePath) {
            try {
