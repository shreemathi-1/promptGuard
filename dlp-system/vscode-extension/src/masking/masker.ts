...
export type MaskStyle = 'REDACT' | 'PARTIAL' | 'TOKENIZE';
...
export class Masker {
  ...
  mask(
    ...
    const replacements: MaskResult['replacements'] = [];
   ...
   */
  private buildReplacement(value: string, category: string, style: MaskStyle): string {
    switch (style) {
      case 'REDACT':
   ...
   */
  maskSingle(value: string, category: string, style: MaskStyle = 'PARTIAL'): string {
